node {
  ...
  wrap([$class: 'AnsiColorBuildWrapper']){
    ...
    stage('Execution'){
      try {
	      bat 'docker exec PoshCore_%BUILD_ID% powershell.exe -NonInteractive -c "$PsVersionTable;$ErrorActionPreference=\'Stop\';cd C:\\PowerShell;$buildConfiguration=\'Release\';$env:chocolateyUseWindowsCompression='\false\';Import-Module .\\tools\\Appveyor.psm1;Invoke-AppveyorInstall"'
